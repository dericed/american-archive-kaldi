fprintf(stderr,
	...
	"\t -raw <B(ig endian) |  L(ittle endian) | M|U (mu law)> (raw binary, native==%s)\n"
	"\n"
	" OUTPUT FORMAT\n"
	...
	"\t -FIR <FIR input filter file (native binary float32) def=(none)>\n"
	...
	"** compiled %s %s **\n",
	S->argv[0],
	(ENDIAN_NATIVE==ENDIAN_BIG)?"Big Endian":"Little Endian",
