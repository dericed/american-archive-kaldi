...
int32 main(int32 argc,char *argv[])
{
  ...
  S->FIR = (FIRcase_t *) malloc(sizeof(FIRcase_t));
...
*/
  ...
  /* clean up the FIR  fir_init takes care of rest */
  S->FIR->fn = NULL;
  ...
  while (argp < S->argc && !argerr)
      ...
      if (S->argv[argp][0] == '-')
	{
	  ...
	  if (!strcmp(arg,"verbose"))
	    ...
	    S->dcremove = 1;
	  else if (!strcmp(arg,"FIR"))
	    S->FIR->fn = strdup(S->argv[++argp]);
...
#define RAW(x) ((x==FORMAT_IN_big)?"Big Endian":(x==FORMAT_IN_little)?"Little Endian":(x==FORMAT_IN_ulaw)?"Mu-Law":"none")
  ...
  fprintf(stderr,"\t FIR input filter:%s\n",
	  IS(S->FIR->fn));
...
int32 convert_setup(suitcase_t *S,parmfile_t *F)
{
  ...
  if (F->fn_ctl == NULL && (*(F->fn_in) == 0 || *(F->fn_out) == 0))
    {
      fprintf(stderr," %s: ERROR\n\tspecify either a control file or input/output files\n",S->argv[0]);
      return(-1);
    }

  if (F->fn_ctl !=NULL && *(F->fn_in) != 0)
    {
  ...
  /* init and load the FIR filter if we have one */
  if (S->FIR->fn != NULL)
    {
	...
	return(-1);
      ...
      if (S->verbose)
	fprintf(stderr," %s: INFO\n\t read %d points from FIR input filter %s\n",
		S->argv[0],S->FIR->hlen,S->FIR->fn);
    }
...
int32 convert_files(suitcase_t *S)
{
  ...
  while (!file_getnextfile(F))
      ...
      while(nread = file_readblock(F))
	{
	  ...
	  /* FIR if necessary (kinda ugly, convert to floats and back, but safe) */
	  if (S->FIR->fn != NULL)
	    fir_int16(S,F->buf_in,F->count_bufin_lastread);
